...
## [Current: v3.1] - 2026-07-02

### 🔄 範圍調整：移除跨門市共用模板
- v3.0 曾規劃並實作「其他門市共用模板」（可瀏覽/複製跨門市自訂模板，含 40 家示範門市假資料），最終決定**移除**，聚焦系統內建模板，讓使用者專注於門市自己的模板管理
- 未來若有更好的模板組合，走法是直接擴充系統內建模板清單，不再透過跨門市瀏覽/複製機制
- 移除範圍：刪除 `prototypes/shared/ext-templates-data.js`；三個原型頁的 ext 相關程式碼（含 `模板簡訊設定教學.html` 的「其他門市」accordion UI）；`簡報.html`／`後端簡報.html`／`流程圖.html` 對應的規格段落，改為記錄此範圍決策
...
### ✨ 新功能：單一模板 → 多模板擴增
- **模板管理頁**（模板簡訊設定教學.html）：CRUD、釘選、啟用/停用、門市預設
