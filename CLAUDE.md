...
* **事件背景**：在早期規劃文件（如 `docs/擴增需求/後端規格書（簡易版）.md` 舊版）中，曾設計過「跨店複製模板」(`POST /api/sms-templates?source_id=...`) 與「跨店瀏覽」(`GET /api/sms-templates` 回傳所有門市自訂模板) 功能。
...
* 後端規格（工程用，完整版）：`docs/擴增需求/後端規格書.md`
* 後端規格（工程用，簡易版）：`docs/擴增需求/後端規格書（簡易版）.md`
