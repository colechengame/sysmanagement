...
### ⚠️ [2026-07-15] 99－虛擬會員新客資料與簡訊規則 (Critical Scope Change)
* **判斷值／範例代碼**：會員的新客來源為 `99－虛擬會員`，`HQ0000146` 為範例會員編號。
* **資料取值**：預約操作、預約查詢、簡訊收件人與詳情均改取該筆預約的「新客姓名／新客電話」；簡訊送至新客電話，不得使用虛擬會員代碼綁定的會員姓名／電話。
* **重複預約例外**：同一虛擬會員代碼可在同一天、同一門市建立多筆不同新客預約；不得只依虛擬會員代碼判斷為同一會員，也不套用一般會員的「同日僅最早一筆可操作」限制。
* **Token 異動**：系統參數 Token 白名單與所有文件／原型均移除會員姓名 Token。
